...
![SABR parameter sweep — animated](figures/sabr_animation.gif)
...
A few representative figures generated by the notebooks (full set in
`figures/`).

| | |
|---|---|
| ![Single SPY fit at three β](figures/fig08_calibration_fit.png) | ![SABR vs flat BS](figures/fig09_rmse_comparison.png) |
| **Single-smile fit** — SPY 30-DTE on 2022-01-03, calibrated three times for β ∈ {0, 0.5, 1}. The three curves are visually indistinguishable: the *β-degeneracy* on a single smile. | **Static fit accuracy** — SABR vs flat Black-Scholes RMSE across all 210 market smiles. Every point lies below the diagonal; median ratio ≈ 18×. |

| | |
|---|---|
| ![Calibrated parameters 2014-2023](figures/fig16_calibration_timeseries.png) | ![Empirical backbone](figures/fig17_backbone_empirical.png) |
| **Time series of (α, ρ, ν)** — fitted to SPY 30-DTE smiles over the full 2014–2023 sample. α spikes at 2018-Q1, COVID 2020, and 2022 selloff; ρ stays in the −0.5 to −0.7 range throughout. | **Empirical backbone** — log-log regression of σ_ATM on F yields slope ≈ +0.18 (SPY) / +0.27 (QQQ), implying β ≈ 1.2 for equity ETFs. |

| | |
|---|---|
| ![Joint sensitivity heatmap](figures/fig14_joint_heatmap.png) | ![Dynamic prediction test](figures/fig10_dynamics_test.png) |
| **Joint (ρ, ν) sensitivity** — left: ATM slope is dominated by ρ (vertical iso-contours); right: ATM curvature is dominated by ν (horizontal iso-contours). The two parameters are nearly orthogonal degrees of freedom. | **Dynamics test** — next-week median prediction RMSE on five SPY date pairs. SABR is the best model-based rule; sticky-strike wins overall on this short horizon because the smile barely moves. |

| | |
|---|---|
| ![Stress event calendar](figures/fig23_stress_event_calendar.png) | ![Portfolio stress replay](figures/fig25_portfolio_stress_replay.png) |
| **Event-anchored stress events** — 32 single-day SPY drops ≤ −3% identified between 2014 and 2023; the worst was −11.5% on 2020-03-16. | **Portfolio scenario replay** — short 100×SPY 95%-strike 30-DTE put repriced under each event's calibrated SABR smile; long 90%-strike put hedge covers a median 60% of the loss. |
...
│   ├── event_stress.py               Event-anchored stress replay
│   └── interactive_viz.py            Animated GIF + Plotly HTML generators
│
...
│   └── test_core.py                  Parity, IV round-trip, ATM, recovery
│
├── figures/                          Generated figures, dashboard screenshots, and animation outputs
├── docs/
│   ├── banner.png                    README banner
│   ├── data_manifest.md              External-data acquisition guide
│   ├── event_stress_replay_explorer.html  Interactive event stress replay dashboard
│   ├── event_stress_replay_data.csv       Data extract for dashboard
│   ├── interactive_stress.html            Plotly stress replay hedge slider
│   ├── interactive_smile.html        Plotly: live SABR α/β/ρ/ν sliders
│   └── interactive_timeseries.html   Plotly: hover-able 2014-2023 calibration
...
A small unit-test suite (`tests/test_core.py`) covers the four
analytical identities the implementation must satisfy: Black put-call
parity, Black implied-volatility round-trip, SABR (2.17) at K=F vs the
closed-form ATM limit (2.18), and synthetic SABR parameter recovery on
a noiseless smile. Run with:
...
All tests run in well under a second and require no market data.

## Reproducing the empirical results

The full reproduction pipeline depends on raw market data that is **not
checked in** (`Data/`) and the cached parquet artifacts that derive
from it (`cache/*.parquet`, ~125 MB). See
[`docs/data_manifest.md`](docs/data_manifest.md) for exact source URLs,
file naming, expected row counts, and a step-by-step checklist.
...
The repo as published is therefore *self-contained for the code, tests,
and rendered figures*, but does **not** include enough on its own to
re-run the calibration without the user supplying the raw inputs above.
